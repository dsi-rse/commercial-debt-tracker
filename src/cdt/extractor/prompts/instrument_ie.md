...
- `start_date` is the date the instrument came into existence: the closing, issuance, or effective date the document states for it. When several dates could anchor it, prefer them in this order: the instrument's own stated dated, issuance, or closing date; then the date the filing says the agreement was entered into. The `dated as of` date of a base indenture, pooling and servicing agreement, purchase agreement, or amendment is never the instrument's start date unless the text says the instrument itself carries that date, and a pricing or announcement date is never a start date.
- In an amended and restated agreement, a facility the text describes as `existing` keeps its original date when the text states one, and otherwise gets no `start_date`. The restatement date belongs only to the instrument as amended and restated, never to the pre-existing facilities it carries forward.
- A draw or advance under an existing note or facility does not restate that instrument's `start_date`: the instrument keeps its own stated date, not the draw date.
...
- A single debt instrument should have at most one start date and at most one `commitment` or `principal` entry. If the document presents two different start dates, or two different commitment or principal figures for what looks like one instrument, that is strong evidence there are two separate debt instruments and you should return two objects. Different `kind` entries — a commitment plus a balance plus a repayment — describe one instrument and never force a split.
- An increase *by* an amount is not a second instrument, and never an object sized at the increment. `increased the commitments by $353 million` and a `$500,000 Credit Increase` describe a change to one facility. What to return depends on which totals the text states:
  - Before and after totals stated: the predecessor/amended pair, each with its stated total.
  - Only the before total stated (`its existing $200 million facility ... increased by $50 million`): still the predecessor/amended pair. The predecessor carries the stated prior total. The instrument as amended carries the arithmetic result as its `commitment` — `normalized_amount` `250000000` — citing **both** the prior-total span and the increment span as `evidence`. Summing the exact cited spans is the only arithmetic you may ever do; never carry the pre-increase total on the instrument as amended.
  - Only the increment stated: one object, and omit the increment from `amounts`.
  An increase *to* an amount is the amended total and belongs on the instrument as amended.
...
- If an amendment increases the commitments under a company's `existing $200 million revolving credit facility` by `$50 million` and never states the new total, return two objects: the predecessor with a `commitment` entry of `200000000`, and the instrument as amended with `status_event` `amended` and a `commitment` entry of `250000000` citing both the `$200 million` and `$50 million` spans. The amended facility's capacity is the sum; publishing the pre-increase `200000000` on it would be wrong.
