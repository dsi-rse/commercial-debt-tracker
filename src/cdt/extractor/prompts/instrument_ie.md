...
- An increase *by* an amount is not a second instrument. `increased the commitments by $353 million` and a `$500,000 Credit Increase` describe a change to one facility: when the text also states the before or after total, return the predecessor/amended pair with those totals, and when it states only the increment, return one object and omit the increment from `amounts`. Never return an object sized at the increment. An increase *to* an amount is the amended total and belongs on the instrument as amended.
