...
- `status`: What this mention says happened to the instrument: `announced`, `entered_into`, `amended`, `terminated`, `repaid`, `exchanged`, or `defaulted` (#141). Null when the mention states no event. `matured` is never extracted; the matcher derives it.
- `status_date`: The date the status event happened or takes effect, when stated.
...
- `parties_json`: JSON array of every party cluster with `canonical_name` (longest span), `role` (`lender`, `borrower`, `agent`, `trustee`, `underwriter`, `guarantor`, `other`), `kind` (`named` or `collective`), and evidence `spans` (#150). Borrower and collective clusters persist; nothing the model labels is discarded.
...
- `dates_json`: JSON array of kind-typed date facts: `{kind, normalized_date, precision, prior, spans, derived_from}` with `kind` one of `agreement` (the instrument's own dated-as-of date), `announcement` (pricing, launch, commitment letter), `expected_closing` (a projected close, never a start), `closing` (closing, issuance, funding, effective — the start), `maturity`, `commitment_termination`. `precision` is `day`, `month` or `year` for how precisely the cited text states the date (`due 2028` normalizes to `2028-12-31` at year precision). `prior` marks a term stated as it stood before an amendment. The flat `start_date`, `maturity_date` and `commitment_termination_date` columns are the current (non-prior) `closing`, `maturity` and `commitment_termination` facts; an `announced` instrument has an `expected_closing` fact and no `closing`, so its `start_date` is null. Responses in the pre-dates[] shape replay with the kind implied by the old property name.
...
- `lenders_known_incomplete`: Boolean flag that is true when the mention shows the document referred to lenders it did not name — collective phrases, public-market placements, redemption notices, assumed debt, and arranger-only syndicates. A false value with no lender parties means the document named no counterparty and signalled none.
